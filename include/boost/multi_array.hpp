...
namespace boost {
      ...
      , private boost::empty_value<Allocator>
    {
     ...
     public:
        ...
        explicit multi_array(Allocator const& alloc = Allocator())
          : super_type(
                (T*)initial_base_
              , c_storage_order()
              , /*index_bases = */0
        ...
        multi_array(multi_array const& rhs)
          : super_type(rhs), alloc_base(static_cast<alloc_base const&>(rhs))
        {
            this->allocate_space();
            ...
            )
        {
            ...
            std::transform(
              ...
              , boost::detail::multi_array::populate_index_ranges()
            );
            std::transform(
     ...
     private:
        ...
        void allocate_space()
        {
            this->base_ = allocator().allocate(this->num_elements());
            this->set_base_ptr(this->base_);
            this->allocated_elements_ = this->num_elements();
            boost::detail::multi_array::construct(
                this->allocator()
              , this->base_
              , this->base_ + this->allocated_elements_
            );
        }
        ...
        void deallocate_space()
        {
            if (this->base_)
            {
                boost::detail::multi_array::destroy(
                    this->allocator()
                  , this->base_
                  , this->base_ + this->allocated_elements_
                );
        ...
        friend inline bool
            operator==(multi_array const& lhs, multi_array const& rhs)
        {
